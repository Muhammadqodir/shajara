    ...
    return (
        <form onSubmit={submit} className="space-y-4">
            <div className="space-y-1.5">
                <Label htmlFor="image_url">Rasm havolasi</Label>
            ...
            {data.image_url ? (
                <div className="bg-secondary aspect-video w-full overflow-hidden rounded-md border">
                    <img src={data.image_url} alt="Ko'rib chiqish" className="h-full w-full object-cover" />
                </div>
            ) : null}

            <div className="space-y-1.5">
                <Label htmlFor="description">Tavsif</Label>
                <Textarea id="description" value={data.description} onChange={(e) => setData('description', e.target.value)} placeholder="Bu qachon va qayerda olingan?" />
                <InputError message={errors.description} />
            </div>

            <div className="space-y-1.5">
                <Label>Belgilangan a'zolar</Label>
                {members.length === 0 ? (
                    <p className="text-muted-foreground text-sm">Belgilash uchun avval oila a'zolarini qo'shing.</p>
                ) : (
            ...
            <DialogFooter>
                <Button type="submit" disabled={processing}>
                    ...
                    {mode === 'edit' ? "O'zgarishlarni saqlash" : "Surat qo'shish"}
                </Button>
            </DialogFooter>
        </form>
    );
    ...
    return (
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-lg">
                <DialogHeader>
                    <DialogTitle>{mode === 'edit' ? 'Suratni tahrirlash' : "Surat qo'shish"}</DialogTitle>
                    <DialogDescription>Surat havolasini kiriting va undagi oila a'zolarini belgilang.</DialogDescription>
                </DialogHeader>
